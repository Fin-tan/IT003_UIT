...
{
    priority_queue<pair<int, int>, vector<pair<int, int>>, compare> Dopen;
    vector<bool> Dclose(v, false);
    map<int, int> parent;
    ...
    while (!Dopen.empty())
    {
        ...
        for (int q = 0; q < v; ++q)
        {
            if (adjacency_matrix[p][q] > 0 && Dclose[q] == false)
            {
                int Dnew = d[p] + adjacency_matrix[p][q];
                if (Dnew < d[q])
                {
                    d[q] = Dnew;
                    parent[q] = p;
